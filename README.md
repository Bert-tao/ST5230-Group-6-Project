...
### If time is limited
Use the current results directly for:
- final PPT
- final report
- qualitative discussion on validity + stability + comment focus

### If you want one more improvement
For **Claude**, rerun `main` and `swap` on the 45-item stability subset, then recompute:
- swap consistency
- prompt consistency

This would make Claude’s effective sample sizes closer to the designed 45-item stability subset.

---

## 8. Project contribution summary

This project contributes:

1. A **task-specific evaluation** of LLM-as-a-Judge on literary translation comparison.
2. A **clear human reference signal** based on PAR3 `hum_gt` high-confidence preference labels.
3. A combined study of:
   - **validity**
   - **answer-order robustness**
   - **prompt robustness**
4. A **comment analysis** layer that compares human and LLM evaluation focus.
5. A reusable pipeline for future judge benchmarking on pairwise comparison tasks.

---

## 9. Contact / collaboration note

When updating the project, please keep filenames consistent and avoid renaming core outputs unless the corresponding scripts and slide references are updated at the same time.
